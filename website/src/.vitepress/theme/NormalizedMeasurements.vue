...
const describe = (row:any)=>`${row.version} · ${names[row.metric]}: ${row.value} ${row.unit} · ${row.ratio===null?'ratio unavailable':row.ratio.toFixed(3)+'× minimum'}${row.normalization_status==='both_zero'?' (zero throughout; shown at 1 as unchanged)':''}`
...
onMounted(async()=>{try{const response=await fetch(withBase(root+'normalized.json'));if(response.ok)plot.value=(await response.json()).plot}catch{/* Static measured figure remains available. */}})
</script>

<template>
  <section class="normalized-measurements" aria-label="Overlaid Bibliography measurements normalized by minimum">
    <template v-if="plot">
      ...
      <div class="normalized-scroll">
        <svg viewBox="0 0 760 340" role="img" aria-label="Four overlaid curves; the minimum of each measurement is one">
          ...
          <text v-for="version in plot.options.versions" :key="version" :x="x(version)" y="305" text-anchor="middle">{{ version }}</text>
          <text x="390" y="330" text-anchor="middle">Bibliography version</text>
    ...
    <img v-else :src="withBase('/examples/bibliography/figures/normalized.svg')" alt="Four Bibliography metrics overlaid across nine versions, each normalized by its minimum." />
    <p class="normalization-note">Each curve has its own minimum at <strong>1</strong>. A value of <strong>1.5</strong> means 50% more than that minimum. The minima can belong to different versions. GC is zero in all recorded samples, so its curve is shown at 1 as unchanged; this is a display convention for equal zeros.</p>
    <a :href="withBase(root+'normalized.json')" download>Download the plotted values</a>
    · <a :href="withBase('/examples/bibliography/figures/normalized.svg')">Open the full-size plot</a>
  </section>
</template>
