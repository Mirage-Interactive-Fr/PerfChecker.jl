(() => {
  ...
  const state = { plan: null, selected: [], dragged: null, series: [], comparison: null,
    plots: [], results: [], currentRun: null, plotRequest: 0, resultRequest: 0,
  ...
  function workloadName(run) { return run.workload || run.feature; }
  function runText(run) { return `${run.package} ${workloadName(run)} ${run.feature} ${run.version} ${run.backend}`.toLowerCase(); }
  ...
  function compareRuns(left, right) {
    ...
    const byFeature = workloadName(left).localeCompare(workloadName(right));
  ...
  function populatePlanFilters() {
    ...
    setOptions($('#feature-filter'), Array.from(new Set(runs.map(workloadName))).sort(), 'All features');
  ...
  function runMatches(run) {
    ...
    if (feature && workloadName(run) !== feature) return false;
    for (const [id, key] of [['#package-filter', 'package'], ['#backend-filter', 'backend']]) {
  ...
  function runCard(run, selected) {
    ...
    const title = document.createElement('strong'); title.textContent = `${run.package} · ${workloadName(run)}`;
  ...
  function resultButton(run) {
    ...
    const detail = document.createElement('small'); detail.textContent = `${run.profile || ''} · ${run.state} · ${run.verdict || 'legacy'} · ${String(run.run_id).slice(0, 8)}`;
  ...
  function resultText(run) {
    ...
    return `${run.suite || ''} ${run.profile || ''} ${run.state || ''} ${run.verdict || ''} ${run.run_id || ''} ${run.finished_at || ''} ${planText}`.toLowerCase();
  }
  function resultMatches(run) {
    ...
    for (const [id, key] of [['#result-suite-filter', 'suite'], ['#result-profile-filter', 'profile'], ['#result-state-filter', 'state'], ['#result-verdict-filter', 'verdict']]) {
  ...
  function populateResultFilters() {
    ...
    setOptions($('#result-verdict-filter'), unique('verdict'), 'All verdicts');
  }
  ...
  function qualificationDetails(bundle) {
    const records = bundle.manifest?.run_qualifications || [];
    if (!records.length) return null;
    const details = document.createElement('details'); details.className = 'qualification-details';
    const heading = document.createElement('summary'); heading.textContent = `Qualification details (${records.length})`; details.append(heading);
    const table = document.createElement('table');
    const head = document.createElement('thead'); const headRow = document.createElement('tr');
    ['Feature', 'Target', 'Probes', 'Correctness', 'Verdict', 'Source'].forEach(label => { const th = document.createElement('th'); th.textContent = label; headRow.append(th); });
    head.append(headRow); table.append(head);
    const body = document.createElement('tbody');
    records.forEach(item => {
      const evidence = item.evidence || {}; const source = evidence.source_provenance || {};
      const probes = (evidence.probes || []).map(probe => `${probe.id || probe.function}: ${probe.status}`).join(', ') || 'none';
      const values = [item.case_id || '', item.target_id || '', probes, evidence.correctness?.status || 'not_checked', evidence.verdict || 'executed', source.captured ? `${String(source.revision || '').slice(0, 12)}${source.dirty ? ' dirty' : ''}` : 'not captured'];
  ...
  function renderPlotPicker(preferredId = '', initialVersion = '') {
    ...
    } else { state.plotRequest += 1; $('#makie-frame').hidden = true; $('#plot-loading')?.remove(); $('#comparison-table').replaceChildren(); }
  }
  async function showPlot(resetVersion = false) {
    const request = ++state.plotRequest;
    ...
    const frame = $('#makie-frame'); frame.hidden = true; frame.loading = 'eager';
    let loading = $('#plot-loading');
    if (!loading) { loading = document.createElement('p'); loading.id = 'plot-loading'; loading.className = 'muted'; loading.setAttribute('role', 'status'); frame.before(loading); }
    loading.textContent = `Loading ${plot.label.toLowerCase()}…`;
    ...
    try {
      ...
      if (request !== state.plotRequest || runId !== state.currentRun) return;
      const versions = payload.options?.selected_version ? (payload.options.versions || []) : [];
      if (versions.length) {
      ...
      frame.onload = () => { if (request === state.plotRequest) { frame.hidden = false; loading.remove(); } };
    ...
    } catch (error) { if (request === state.plotRequest) { loading.textContent = 'The plot could not be loaded.'; toast(error.message); } }
  }
  async function loadComparison(id) {
    ...
    try {
      ...
      [`${bundle.manifest.suite}`, `${bundle.manifest.verdict || 'legacy'} verdict`, `${comparison.verdict || 'inconclusive'} comparison`, `${state.series.length} series`, `${comparison.records.length} comparisons`, `${state.plots.length} Makie plots`, `${bundle.observations.length} observations`].forEach(text => { const strong = document.createElement('strong'); strong.textContent = text; summary.append(strong); });
      const details = qualificationDetails(bundle); if (details) summary.append(details);
      const adviceButton = document.createElement('button'); adviceButton.type = 'button'; adviceButton.textContent = 'Evidence-based advice';
      const advicePanel = document.createElement('section'); advicePanel.className = 'panel'; advicePanel.hidden = true;
      adviceButton.addEventListener('click', async () => {
        try {
          const report = await api(`/advice?id=${encodeURIComponent(id)}`);
          advicePanel.replaceChildren(); advicePanel.hidden = false;
          for (const item of report.recommendations) {
            const article = document.createElement('article');
            for (const [label, value] of [['Observation', item.hypothesis], ['Next experiment', item.action], ['Verify afterwards', item.validation]]) {
              const p = document.createElement('p'); const strong = document.createElement('strong'); strong.textContent = `${label}: `; p.append(strong, document.createTextNode(value)); article.append(p);
            }
            const evidence = document.createElement('details'); const title = document.createElement('summary'); title.textContent = 'Evidence and limits';
            const pre = document.createElement('pre'); pre.textContent = JSON.stringify({evidence: item.evidence, limitations: item.limitations}, null, 2); evidence.append(title, pre); article.append(evidence); advicePanel.append(article);
          }
          if (!report.recommendations.length) advicePanel.textContent = 'No recommendation supported by these rules. This does not certify optimal performance.';
        } catch (error) {toast(error.message);}
      });
      summary.append(adviceButton, advicePanel);
      $('#result-summary').replaceWith(summary); summary.id = 'result-summary';
      populatePlotFilters();
      const hash = new URLSearchParams(location.hash.slice(1));
      const preferred = hash.get('run') === id ? hash.get('plot') : (state.plots.find(plot => plot.kind === 'normalized_metrics')?.id || state.plots.find(plot => plot.kind === 'version_series' && plot.metric === 'julia.wall.time')?.id);
      const picker = $('#plot-picker');
  ...
  function startStudio() {
    ...
    ['#result-suite-filter', '#result-profile-filter', '#result-state-filter', '#result-verdict-filter', '#result-sort'].forEach(id => $(id).addEventListener('change', renderResults));
    ...
    $('#clear-result-filters').addEventListener('click', () => {
      $('#result-filter').value = '';
      ['#result-suite-filter', '#result-profile-filter', '#result-state-filter', '#result-verdict-filter'].forEach(id => $(id).value = '');
