...
function _restore_workspace!(workspace::StudioWorkspace)
    ...
    payload = PerfChecker._json_parsefile(workspace.state_path)
...
function _agent_request(server::String, path::String, token::String;
    ...
    parsed = isempty(body) ? Dict{String, Any}() : PerfChecker._json_parse(body)
