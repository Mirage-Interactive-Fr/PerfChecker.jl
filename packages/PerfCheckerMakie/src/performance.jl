...
function _normalized_metrics_figure(plot)
    isempty(plot.data) && return _empty_performance_figure(plot)
    figure, axis = _performance_figure(plot.title)
    ...
    for (i, metric) in pairs(metrics)
        ...
        any(row -> row["normalization_status"] == "both_zero", rows) &&
            (label *= " (0/0: unchanged)")
        scatterlines!(axis, xs, ys; label, color = colors[i], linewidth = 2,
...
function PerfChecker.performance_figure(plot::PerfChecker.PerformancePlot)
    plot.kind === :normalized_metrics && return _normalized_metrics_figure(plot)
...
function PerfChecker.performance_figure(bundle::PerfChecker.RunBundle; kwargs...)
    return PerfChecker.performance_figure(PerfChecker.performance_plot(bundle; kwargs...))
end
