@testitem "Feature and software suites" tags=[:suites] begin
    ...
    using Oxygen
    using PProf
    using PerfChecker, PerfCheckerWeb, PerfCheckerPluto, PerfCheckerMakie
    using Pluto
    using PropCheck
    using Supposition
    using UnicodePlots
...
""")
        ...
        package = PackageSuite("Example";
            worker_environment = dir, source = dir, versions = :all,
            features = [always, recent])
        @test worker_environment(package) == dir
        @test package.environment == dir
        @test PackageSuite("Example"; environment = dir, worker_environment = dir,
            source = dir, versions = VersionNumber[], include_dev = false,
            features = [always]).worker_environment == dir
        other_environment = mkpath(joinpath(dir, "other-worker"))
        @test_throws ArgumentError PackageSuite("Example"; environment = dir,
            worker_environment = other_environment, source = dir,
            versions = VersionNumber[], include_dev = false, features = [always])

        contaminated = mkpath(joinpath(dir, "contaminated-worker"))
        write(joinpath(contaminated, "Project.toml"), """
[deps]
PerfChecker = "6309bf6b-a531-4b08-891e-8ee981e5c424"
""")
        contaminated_package = PackageSuite("Example";
            worker_environment = contaminated, source = dir,
            versions = VersionNumber[], include_dev = false, features = [always])
        @test_logs (:warn, r"directly depends on PerfChecker") plan_suite(
            SoftwareSuite(:contaminated, [contaminated_package]);
            profile = :historical, version_provider = _ -> VersionNumber[])
        @test_throws ArgumentError PerfChecker._validate_worker_environments(
            [contaminated_package]; strict = true)
        self_hosted = PackageSuite("PerfChecker";
            worker_environment = contaminated, source = pkgdir(PerfChecker),
            versions = VersionNumber[], include_dev = false, features = [always])
        @test isnothing(PerfChecker._worker_environment_issue(self_hosted))
        ...
        @test suite_verdict(candidate_result) == :executed
        @test candidate_config[].options[:target_install] === :add
        @test candidate_config[].options[:devops].rev == "feature/fast-parser"

        function invalid_candidate_runner(_, _, _, _)
            qualification = Dict{String, Any}(
                "probes" => Dict{String, Any}[],
                "correctness" => Dict{String, Any}(
                    "status" => "failed", "message" => "state hash differs"))
            return PerfChecker.CheckerResult(
                PerfChecker.Table[PerfChecker.Table(times = [1.0])], nothing,
                [:candidate],
                Pkg.Types.PackageSpec[Pkg.Types.PackageSpec(
                    name = "Example", version = v"0.2.0")],
                Dict{String, Any}[qualification])
        end
        invalid_result = run_suite(candidate_plan;
            executor = invalid_candidate_runner, strict = false)
        @test only(invalid_result.runs).status === :invalid
        @test suite_verdict(invalid_result) === :invalid
        @test !suite_passed(invalid_result)

        function resource_regression_runner(_, _, _, _)
            qualification = Dict{String, Any}(
                "probes" => Dict{String, Any}[],
        ...
        resource_regression = run_suite(candidate_plan;
            executor = resource_regression_runner, strict = false)
        resource_run = only(resource_regression.runs)
        @test resource_run.status === :invalid
        @test resource_run.result isa PerfChecker.CheckerResult
        @test occursin("external live bytes grew", resource_run.message)

        function blocked_candidate_runner(_, _, _, _)
            qualification = Dict{String, Any}(
                "probes" => [Dict{String, Any}(
                "id" => "jolt", "status" => "unavailable",
                "blocking" => true, "message" => "symbol missing")])
            return PerfChecker.CheckerResult(
                PerfChecker.Table[PerfChecker.Table(times = [1.0])], nothing,
                [:candidate],
                Pkg.Types.PackageSpec[Pkg.Types.PackageSpec(
        ...
        blocked_result = run_suite(candidate_plan;
            executor = blocked_candidate_runner, strict = false)
        @test only(blocked_result.runs).status === :blocked
        @test suite_verdict(blocked_result) === :blocked
        ...
        parsed = PerfChecker._json_parsefile(joinpath(dir, "reports", "suite-result.json"))
...
""")
        ...
        @testset "Oxygen extension" begin
            ...
            @test occursin("result-verdict-filter", studio_html)
            ...
            @test any(job -> job["job_id"] == launch_body["job_id"],
                PerfChecker._json_parsefile(state_file)["jobs"])
            Oxygen.resetstate()
...
""")
            ...
            @test makie_response.status == 501
            @test occursin("WGLMakie", String(makie_response.body))
            Oxygen.resetstate()
        ...
        @testset "Pluto and Makie extensions" begin
            ...
            documenter_path = documenter_page(bundle,
                joinpath(dir, "website", "performance.md"))
            ...
            filtered_documenter_path = documenter_page(bundle,
                joinpath(dir, "website", "filtered-performance.md"); blocks = [block])
