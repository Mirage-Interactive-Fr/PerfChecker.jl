...
function _cli_sample_statistics(options)
    statistics = Dict{String, Symbol}()
    for item in _cli_values(options, "statistic")
        metric, statistic = split(item, '='; limit = 2)
        statistics[metric] = _normalize_sample_statistic(statistic)
    end
    return statistics
end
...
function _cli_candidates(options)
    _candidates_from_payloads(
        (_json_parse(item) for item in _cli_values(options, "candidate")), "--candidate")
end
...
function _cli_comparisons(options)
    ComparisonPolicy[_comparison_from_payload(_json_parse(item), "--comparison")
...
Commands:
  ...
  discover         Suggest scenarios from tests and literal CI matrices (no execution)
  diagnose         Run optional analyzers for --catalog in isolated processes
  advise           Explain a saved --source diagnosis or --bundle (no execution)
  tools            Inventory integrations and candidates without executing them
  testitems        Measure existing @testitem blocks; --list discovers without execution
  machine          Print the current machine specification class as JSON
  native-plan      Prepare optional profiler arguments with --tool and --script
...
function perfchecker_main(args = ARGS; stdout::IO = Base.stdout,
    ...
    try
        if command == "native-plan"
            JSON.print(stdout,
                native_tool_plan(Symbol(_cli_required(options, "tool")),
                    _cli_required(options, "script");
                    project = _cli_value(
                        options, "project", dirname(Base.active_project())),
                    output = _cli_value(options, "output", "native-profile"),
                    julia = _cli_value(options, "julia", first(Base.julia_cmd().exec)),
                    suppressions = _cli_value(options, "suppressions")),
                2)
            return 0
        end
        if command in ("machine", "estimate")
            payload = if command == "machine"
                machine_profile(; label = _cli_value(options, "label", ""))
            else
                input = _json_parsefile(_cli_required(options, "source"))
                estimate_performance(
                    input["target"], input["references"], input["workload"])
            end
            output = _cli_value(options, "output")
            if output === nothing
                JSON.print(stdout, payload, 2)
            else
                ispath(output) && throw(ArgumentError("output already exists"))
                mkpath(dirname(abspath(output)))
                open(io -> JSON.print(io, payload, 2), output, "w")
            end
            return command == "estimate" && payload["status"] != "estimated" ? 1 : 0
        end
        if command == "testitems"
            Base.find_package("TestItemRunner") === nothing &&
                throw(ArgumentError("Load PerfChecker and TestItemRunner >= 1.3.2 in the controller project"))
            Base.require(Main, :TestItemRunner)
            root = abspath(_cli_value(options, "root", pwd()))
            tags = Symbol.(filter(!isempty, split(_cli_value(options, "tags", ""), ',')))
            exclude_tags = Symbol.(filter(
                !isempty, split(_cli_value(options, "exclude-tags", ""), ',')))
            list = _cli_bool(options, "list")
            payload = if list
                Base.invokelatest(discover_testitems, root; tags, exclude_tags)
            else
                requested = _cli_values(options, "item-id")
                Base.invokelatest(run_testitems, root; tags, exclude_tags,
                    ids = isempty(requested) ? nothing : requested,
                    project = _cli_value(
                        options, "project", dirname(Base.active_project())),
                    samples = Base.parse(Int, _cli_value(options, "samples", "1")),
            ...
            return list || payload["passed"] ? 0 : 1
        end
        if command == "advisor-setup"
            input = _json_parsefile(_cli_required(options, "source"))
            payload = advisor_setup(
                input["config"]; action = Symbol(get(input, "action", "probe")),
                model = get(input, "model", ""), confirmed = get(input, "confirmed", false),
                project = _cli_value(options, "project", dirname(Base.active_project())))
        ...
        if command in ("discover", "diagnose", "advise", "tools", "narrate",
           ...
           (command == "compare" && _cli_bool(options, "scenarios"))
            return _scenario_cli(command, positional, options, stdout)
        end
        ...
        elseif command == "run"
            ...
            println(stdout, uppercase(string(suite_verdict(result))))
        ...
        elseif command in ("compare", "check")
            ...
            comparison = compare_bundles(baseline, candidate;
                ...
                sample_statistics = _cli_sample_statistics(options),
                default_sample_statistic = Symbol(
                    _cli_value(options, "default-statistic", "median")))
            ...
            println(stdout, uppercase(string(comparison_verdict(comparison))))
        ...
        elseif command == "capabilities"
            payload = Dict{String, Any}(
                ...
                "scenario_analyzers" => _scenario_capabilities(),
...
@testitem "Unified CLI" tags=[:unit, :cli] begin
    ...
    _, statistic_options, _ = PerfChecker._cli_split(
        ["--statistic=julia.wall.time=p99", "--statistic=process.rss.delta=max"])
    @test PerfChecker._cli_sample_statistics(statistic_options) ==
          Dict("julia.wall.time" => :p99, "process.rss.delta" => :maximum)
    _, invalid_statistic_options, _ = PerfChecker._cli_split(["--statistic=julia.wall.time=mode"])
    @test_throws ArgumentError PerfChecker._cli_sample_statistics(
        invalid_statistic_options)
