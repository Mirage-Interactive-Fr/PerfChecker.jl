...
"""
Result of an external command executed in a requested network namespace.
Carries the command outcome, captured output, provider and network sample.
Inspect `isolated_network_result_dict` before attributing
traffic to the target; unavailable isolation is not a zero-traffic measurement.
"""
...
@testitem "Network isolation contracts" tags=[:unit, :network] begin
    ...
    @test PerfChecker._json_parsefile(joinpath(pkgdir(PerfChecker), "schemas",
        "perfchecker-network-isolation-spec-v1.schema.json"))["type"] == "object"
    @test PerfChecker._json_parsefile(joinpath(pkgdir(PerfChecker), "schemas",
        "perfchecker-isolated-network-result-v1.schema.json"))["type"] == "object"
