...
function _read_jsonl(path::AbstractString)
    ...
    for line in eachline(path)
        isempty(strip(line)) || push!(records, _json_parse(line))
    end
    return records
end
...
function verify_run_bundle(directory::AbstractString; require_integrity::Bool = false)
    ...
    payload = _json_parsefile(integrity_path)
...
const _RESOURCE_METRIC_DEFINITIONS = Dict{Symbol, Tuple{String, String}}(
    :collector_seconds => ("process.collector.wall_time/v1", "seconds"),
    :rss_before_bytes => ("process.rss.before/v1", "bytes"),
    :rss_after_bytes => ("process.rss.after/v1", "bytes"),
    :rss_delta_bytes => ("process.rss.delta/v1", "bytes"),
    :peak_rss_before_bytes => ("process.rss.peak_lifetime_before/v1", "bytes"),
    :peak_rss_after_bytes => ("process.rss.peak_lifetime_after/v1", "bytes"),
    :new_peak_rss_bytes => ("process.rss.new_lifetime_peak/v1", "bytes"),
    :private_before_bytes => ("process.private.before/v1", "bytes"),
    :private_after_bytes => ("process.private.after/v1", "bytes"),
    :private_delta_bytes => ("process.private.delta/v1", "bytes"),
    :external_live_before_bytes => ("external.live.before/v1", "bytes"),
    :external_live_after_bytes => ("external.live.after/v1", "bytes"),
    :external_live_delta_bytes => ("external.live.delta/v1", "bytes"),
    :external_reserved_before_bytes => ("external.reserved.before/v1", "bytes"),
    :external_reserved_after_bytes => ("external.reserved.after/v1", "bytes"),
    :external_reserved_delta_bytes => ("external.reserved.delta/v1", "bytes"),
    :external_allocated_delta_bytes => ("external.allocated.delta/v1", "bytes"),
    :external_freed_delta_bytes => ("external.freed.delta/v1", "bytes"))

function _resource_definition_dict(id::String, unit::String)
    definition = _definition_dict(id, unit, :process_resources)
    definition["sample_semantics"] = "one envelope around a backend collection"
    definition["warmup_policy"] = "backend controlled; feature setup excluded"
    definition["includes_compilation"] = true
    definition["attribution_scope"] = "isolated_worker_process"
    return definition
end

function _append_resource_records!(definitions, observations, diagnostics,
        envelope::ResourceEnvelope, run_id::String, attempt_id::String,
        case_id::String, target_id::String, planned::PlannedFeatureRun)
    process_status = envelope.process_before.status === :observed &&
                     envelope.process_after.status === :observed
    if !process_status
        message = join(
            filter(!isempty,
                [envelope.process_before.message, envelope.process_after.message]),
            "; ")
        push!(diagnostics,
            Dict{String, Any}(
                "record_type" => "diagnostic", "run_id" => run_id,
                "attempt_id" => attempt_id, "case_id" => case_id,
                "target_id" => target_id, "rule_id" => "resource.process.unavailable",
                "severity" => "info", "message" => message,
                "fingerprint" => _content_digest(("resource.process.unavailable",
                    case_id, target_id, message)),
                "evidence" => resource_envelope_dict(envelope)))
    end
    external_status = envelope.external_before.status === :observed &&
                      envelope.external_after.status === :observed
    if envelope.external_requested && !external_status
        message = join(
            filter(!isempty,
                [envelope.external_before.message, envelope.external_after.message]),
            "; ")
        push!(diagnostics,
            Dict{String, Any}(
                "record_type" => "diagnostic", "run_id" => run_id,
                "attempt_id" => attempt_id, "case_id" => case_id,
                "target_id" => target_id, "rule_id" => "resource.external.unavailable",
                "severity" => "info", "message" => message,
                "fingerprint" => _content_digest(("resource.external.unavailable",
                    case_id, target_id, message)),
                "evidence" => resource_envelope_dict(envelope)))
    end
    attributes = Dict{String, Any}(
        ...
        "backend" => string(planned.feature.backend),
        "process_provider" => envelope.process_after.provider,
        "external_provider" => envelope.external_after.provider,
        "peak_semantics" => "process lifetime high-water mark")
    for (metric, value) in resource_envelope_metrics(envelope)
        value === nothing && continue
        definition_id, unit = _RESOURCE_METRIC_DEFINITIONS[metric]
        definitions[definition_id] = _resource_definition_dict(definition_id, unit)
        push!(observations,
            Dict{String, Any}(
                "record_type" => "observation", "run_id" => run_id,
                "attempt_id" => attempt_id, "case_id" => case_id,
                "target_id" => target_id, "metric" => _metric_name(definition_id),
                "value" => value, "unit" => unit,
                "aggregation" => "collector_envelope", "sample_index" => 1,
                "scope" => "isolated_worker_process", "attributes" => attributes,
...
function _result_protocol_records(result::SoftwareSuiteResult, run_id::String,
    ...
    for run in result.runs
        ...
        if run.status !== :pass
            rule = run.status === :unavailable ? "feature.unavailable" :
                   run.status === :blocked ? "feature.capability_blocked" :
                   run.status === :invalid ? "feature.correctness_failed" :
                   "feature.failed"
        ...
        for (table_index, table) in enumerate(run.result.tables)
            ...
            for column in columns
                ...
                if backend in (
                   ...
                   actual_policy == "fresh"
                    definition_id *= "/fresh-state-evals1-v1"
                end
        ...
        for envelope in run.result.resource_envelopes
            envelope === nothing && continue
            _append_resource_records!(definitions, observations, diagnostics,
                envelope, run_id, attempt_id, case_id, target_id, planned)
        end
    end
...
function _suite_run_bundle(result::SoftwareSuiteResult; run_id = uuid4(),
    ...
    qualifications = [Dict{String, Any}(
                          "case_id" => "$(run.planned.suite)/$(run.planned.package_suite.id)/$(run.planned.feature.id)",
                          "target_id" => run.planned.target.label,
                          "evidence" => run.qualification)
                      for run in result.runs]
    function unique_provenance(key)
        records = Dict{String, Any}()
        for run in result.runs
            haskey(run.qualification, key) || continue
            record = run.qualification[key]
            records[_content_digest(record)] = record
        end
        return [records[id] for id in sort!(collect(keys(records)))]
    end
    source_provenance = unique_provenance("source_provenance")
    environment_provenance = unique_provenance("environment_provenance")
    hardware = Dict{String, Any}[]
    for run in result.runs
        run.result isa CheckerResult || continue
        run.result.hwinfo === nothing && continue
        info = run.result.hwinfo
        record = Dict{String, Any}(
            "id" => hardware_id(info),
            "cpu_name" => info.machine,
            "word_size" => info.word,
            "simd_bytes" => info.simdbytes,
            "physical_cores" => info.corecount[1],
            "logical_threads" => info.corecount[2],
            "threads_per_core" => info.corecount[3])
        any(existing -> existing["id"] == record["id"], hardware) ||
            push!(hardware, record)
    end
    warnings = String[]
    isempty(source_provenance) && push!(warnings,
        "source provenance was not captured by the suite executor")
    any(item -> item isa AbstractDict && !Bool(get(item, "captured", false)),
        source_provenance) && push!(warnings,
        "one or more source targets have no Git working-tree provenance")
    ...
    manifest = Dict{String, Any}(
        ...
        "environment" => Dict(
            ...
            "hardware" => hardware,
            "hardware_ids" => String[String(item["id"]) for item in hardware]),
        "source_provenance" => source_provenance,
        "environment_provenance" => environment_provenance,
        "run_qualifications" => qualifications,
        "collector_capabilities" => sort!(unique!(vcat(
            [string(run.planned.feature.backend) for run in result.runs],
            any(
                run -> run.result isa CheckerResult &&
                    any(!isnothing, run.result.resource_envelopes),
                result.runs) ?
            ["process_resources"] : String[]))),
        "warnings" => warnings)
...
Return whether the manifest is complete and no diagnostic has error severity.
This is an execution check, not a comparison against a performance budget.
Use `compare_bundles` and `comparison_verdict` for a regression decision.
"""
...
Convert a completed suite result into a portable bundle under `root/run-<id>`.
Return the absolute bundle directory. The atomic writer rejects an existing
destination. The evidence label is supplied by the caller; writing a bundle does
not establish that previously cached observations were freshly measured.
"""
...
function read_run_bundle(directory::AbstractString; verify_integrity::Bool = true,
    ...
    manifest = _json_parsefile(joinpath(root, "manifest.json"))
    get(manifest, "schema_version", nothing) == RUN_BUNDLE_SCHEMA ||
        throw(ArgumentError("unsupported run bundle schema"))
    definitions = _json_parsefile(joinpath(root, "measurement-definitions.json"))
    artifacts = _json_parsefile(joinpath(root, "artifacts.json"))
...
Read manifests in child bundle directories and return them with `bundle_path`.
A missing root returns an empty vector. Unreadable or foreign manifests are
ignored. This is discovery only: call `read_run_bundle` to verify bundle integrity
before using its observations.
"""
function list_run_bundles(root::AbstractString; recursive::Bool = false)
    ...
    for entry in entries
        ...
        try
            manifest = _json_parsefile(manifest_path)
...
@testitem "Resource envelopes become run-bundle observations" tags=[:unit, :resources,
    :protocol] begin
    using PerfChecker
    import Pkg
...
version = "0.1.0"
""")
        entrypoint = joinpath(dir, "resource.jl")
        write(entrypoint, "perf_workload(state) = nothing\n")
        feature = FeatureSpec(:resource; entrypoint,
            options = Dict(:samples => 1, :seconds => 0.01))
        package = PackageSuite("ResourceFixture"; worker_environment = dir,
            source = dir, versions = VersionNumber[], features = [feature])
        plan = plan_suite(SoftwareSuite(:resources, [package]); profile = :quick,
            version_provider = _ -> VersionNumber[])
        planned = only(plan.runs)

        before = PerfChecker.ProcessMemorySnapshot(42, 1; rss_bytes = 100,
            peak_rss_bytes = 120, private_bytes = 80, provider = "fixture")
        after = PerfChecker.ProcessMemorySnapshot(42, 2; rss_bytes = 140,
            peak_rss_bytes = 160, private_bytes = 90, provider = "fixture")
        external_before = external_memory_snapshot((;
            ...
            provider = "fixture"))
        external_after = external_memory_snapshot((;
            schema_version = "perfchecker-external-memory/1", live_bytes = 0,
            reserved_bytes = 128, allocated_bytes_total = 96, freed_bytes_total = 96,
            provider = "fixture"))
        ...
        checker = PerfChecker.CheckerResult(
            PerfChecker.Table[PerfChecker.Table(times = [10.0], gctimes = [0.0],
                memory = [0], allocs = [0])], nothing, [:resources],
            Pkg.Types.PackageSpec[Pkg.Types.PackageSpec(
                name = "ResourceFixture", version = v"0.1.0")],
            Dict{String, Any}[Dict{String, Any}()],
            Union{Nothing, ResourceEnvelope}[envelope])
        run = FeatureRun(planned, :pass, 0.25, checker, "",
            Dict{String, Any}())
        result = SoftwareSuiteResult(plan, "start", "finish", [run])
        bundle = PerfChecker._suite_run_bundle(result)

        @test "process_resources" in bundle.manifest["collector_capabilities"]
        definitions = Set(record["id"] for record in bundle.measurement_definitions)
        @test "process.rss.delta/v1" in definitions
        @test "external.live.delta/v1" in definitions
        rss = only(filter(
            record -> record["measurement_definition"] == "process.rss.delta/v1",
            bundle.observations))
        @test rss["value"] == 40
        @test rss["aggregation"] == "collector_envelope"
        @test rss["scope"] == "isolated_worker_process"
    end
end
...
@testitem "Portable run bundle protocol" tags=[:unit, :protocol] begin
    ...
    mktempdir() do dir
        @test PerfChecker._json_parsefile(joinpath(pkgdir(PerfChecker), "schemas",
            "perfchecker-run-bundle-v1.schema.json"))["type"] == "object"
        @test PerfChecker._json_parsefile(joinpath(pkgdir(PerfChecker), "schemas",
            "perfchecker-provider-result-v1.schema.json"))["type"] == "object"
        @test PerfChecker._json_parsefile(joinpath(pkgdir(PerfChecker), "schemas",
            "perfchecker-bundle-integrity-v1.schema.json"))["type"] == "object"
        ...
        invalid = PerfChecker._json_parsefile(provider_path)
        ...
        invalid_unit = PerfChecker._json_parsefile(provider_path)
