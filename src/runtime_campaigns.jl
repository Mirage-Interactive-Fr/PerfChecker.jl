...
function _runtime_failure_kind(execution)
    ...
    elseif occursin("readonlymemoryerror", output) ||
           any(marker -> occursin(marker, output),
        ("failed to precompile", "the following direct dependency failed to precompile",
            "gpucompiler", "alloccheck"))
        return "ecosystem_incompatibility"
...
function _runtime_failure_stage(execution)
    execution.timed_out && return "unknown"
    output = lowercase(string(execution.stderr, '\n', execution.stdout))
    any(marker -> occursin(marker, output),
        ("unsatisfiable requirements", "resolvererror", "resolving package versions")) &&
        return "resolve"
    any(marker -> occursin(marker, output),
        ("readonlymemoryerror", "failed to precompile", "precompiling project",
            "the following direct dependency failed to precompile")) &&
        return "precompile"
    occursin("error: loaderror", output) && return "load"
    return "suite"
end
...
function _runtime_campaign_run(spec::JuliaRuntimeSpec; suite::String, reports::String,
    ...
    failure_stage = status == "passed" ? "complete" :
                    status == "missing_bundle" ? "report" :
                    _runtime_failure_stage(execution)
    return Dict{String, Any}(
        "runtime_id" => string(spec.id), "role" => string(spec.role),
        "spec" => julia_runtime_spec_dict(spec), "probe" => probe,
...
function _runtime_comparison(baseline::Dict{String, Any},
        ...
        min_samples::Integer, sample_statistics::AbstractDict,
    ...
    return Dict{String, Any}(
        ...
        "baseline_runtime_id" => baseline["runtime_id"],
        "candidate_runtime_id" => candidate["runtime_id"],
        "status" => string(comparison_verdict(comparison)),
...
function run_julia_runtime_campaign(specs::AbstractVector{JuliaRuntimeSpec};
    ...
    _normalize_sample_statistic(default_sample_statistic)
    foreach(value -> _normalize_sample_statistic(value), values(sample_statistics))
    ...
    comparisons = [_runtime_comparison(baseline, run;
                       relative_limits, min_samples, sample_statistics,
                       default_sample_statistic) for run in runs if run !== baseline]
...
Write `julia-runtime-campaign.json` and a Markdown summary in `directory` and
return both absolute paths. Existing report files are replaced. Captured process
output is omitted from JSON unless `include_output=true`. No measurements run.
"""
function write_julia_runtime_campaign(campaign::JuliaRuntimeCampaign,
    ...
    open(markdown_path, "w") do io
        ...
        println(io, "| Runtime | Role | Julia | Status | Phase | Duration | Diagnostic |")
        println(io, "| --- | --- | --- | --- | --- | ---: | --- |")
        for run in campaign.runs
            ...
            println(io,
                ...
                "$(run["status"]) | $(get(run, "failure_stage", "—")) | " *
                "$(round(run["elapsed_seconds"]; digits = 2)) s | " *
                "$diagnostic |")
        end
...
@testitem "Julia runtime campaign contract" tags=[:unit, :runtime, :protocol] begin
    ...
    @test PerfChecker._json_parsefile(joinpath(pkgdir(PerfChecker), "schemas",
        "perfchecker-julia-runtime-campaign-v1.schema.json"))["type"] == "object"
    ...
    ecosystem = (timed_out = false, exit_code = 1,
        stderr = "ERROR: ReadOnlyMemoryError()\nFailed to precompile GPUCompiler via AllocCheck",
        stdout = "Precompiling project...")
    @test PerfChecker._runtime_failure_kind(ecosystem) == "ecosystem_incompatibility"
    @test PerfChecker._runtime_failure_stage(ecosystem) == "precompile"
