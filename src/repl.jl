...
Print a readable plan, including target labels and availability status, without
starting workers. `limit` must be positive and bounds displayed rows.
"""
print_suite_plan(plan::SuitePlan; kwargs...) = print_suite_plan(stdout, plan; kwargs...)
...
Prompt for package, workload, collector, version bounds, search and sort order,
then print the selected plan. Read positive sample, duration and worker-thread
settings and return `(plan, overrides)` after confirmation. This prepares a run;
it does not execute workloads or write reports. Pass the returned values to
[`run_suite_repl`](@ref).

Empty selectors include all matching runs; an empty numeric answer uses the
displayed default. Reject an empty selection or invalid settings with
`ArgumentError`; declining confirmation throws `InterruptException`.
`input` and `output` may be redirected for a reproducible scripted dialogue.
"""
function configure_suite_repl(suite::SoftwareSuite; profile::Symbol = :historical,
    ...
    threads > 0 || throw(ArgumentError("worker threads must be positive"))
